...
DefaultData.GetKlineOption = function () {
  let data = {
    ...
    Windows: [
      ...
      { Index: "SAR", Modify: true, Change: true },
    ],
    //Symbol: '600000.sh',
    IsAutoUpdate: true,
...
DefaultData.GetIndexMenu = function () //指标菜单配置
{
  var data =
    ...
    [
      {
        Name: "顶部背离",
        ID: "61c8915d-c456-6ea8-2260-85sd3s6fg5sd",
        WindowIndex: 0,
      },
      {
        Name: "高低区间",
        ID: "61c8915d-c456-6ea8-2260-5sd22df5a3s1",
        WindowIndex: 1,
      },
  ...
  methods: {
    ...
    RequestMinuteRealtimeData(
      ...
      $.ajax({
        ...
        success: (recvData) => {
          this.RecvMinuteRealtimeData(recvData, callback, {
            Name: name,
            Symbol: symbol,
          });
        },
    ...
    RecvMinuteRealtimeData(recvData, callback, stockData) {
      ...
      console.log(
        "[BitKLine::RecvMinuteRealtimeData] hqchartdata",
        hqChartData
      );
      callback(hqChartData);
    },
    ...
    RequestRealtimeData(
      ...
      $.ajax({
        ...
        success: (recvData) => {
          this.RecvRealtimeData(recvData, callback, {
            Name: name,
            Symbol: symbol,
          });
        },
    ...
    JsonToHQChartMinuteHistoryData(
      recvData //分钟（历史/最新）数据转化为hqchart数据格式
    ) {
    ...
    JsonToHQChartHistoryData(
      recvData //日线历史数据转化为hqchart数据格式
    ) {
      var data = recvData.data.series;
      var yClose = null;
      var klineData = [];
    ...
    JsonToHQChartRealtimeData(
      recvData //日线最新数据转化为hqchart数据格式
    ) {
      var stockData = {};
      var data = recvData.data.series;
      var yClose = null;
